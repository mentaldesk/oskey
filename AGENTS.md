...
|-----------|------------------------------------------------------------------------------------|
| `main`    | ZMK `main` — the unreleased development line (currently moving to Zephyr 4.1 / the future 0.4.0) |
| `v0.3.0`  | ZMK `v0.3.0` — latest stable release (Zephyr 3.5). **This branch.**                 |
...
- The `zmkfirmware/zmk-build-arm:<tag>` image in `init-tests` / `run-tests` matches the Zephyr
  version of the targeted ZMK revision (`:3.5` on this branch).
